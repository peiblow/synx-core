...
func (r *Runtime) HandleDeploy(msg *WireMessage) WireResponse {
	...
	artifact, err := Build(string(req.Source), r.baseDir)
	if err != nil {
		return WireResponse{
			Type:    "DEPLOY_RESPONSE",
			ID:      msg.ID,
...
// agentHashFromStorage extrai o hash do agente do InitStorage (computado na
// inicialização). name/version/purpose já vivem no artifact.AgentInfo (compiler).
func agentHashFromStorage(artifact *compiler.ContractArtifact) (string, error) {
	for _, val := range artifact.InitStorage {
		agentMap, ok := val.(map[string]interface{})
		if !ok {
			continue
		}
		if hash, ok := agentMap["hash"].(string); ok && hash != "" {
			return hash, nil
		}
	}

	return "", fmt.Errorf("no agent declaration found in contract storage — did you declare an 'agent' block?")
}
